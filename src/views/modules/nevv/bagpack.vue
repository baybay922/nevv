	...
	<el-table class="userTable" border :data="dataList" highlight-current-row v-loading="listLoading">
		...
		<el-table-column prop="item" label="Item">
				<template slot-scope="scope">
					<div class="whitesSace">{{scope.row.Item}}</div>
				</template>
		</el-table-column> 
		<el-table-column prop="materials" label="Materials">
			<template slot-scope="scope">
				<div class="whitesSace">{{scope.row.materials}}</div>
			</template>
		</el-table-column> 
		<el-table-column prop="predict" label="Predict">
			<template slot-scope="scope">
				<div class="whitesSace">{{scope.row.predict}}</div>
			</template>	
		</el-table-column> 
		<el-table-column prop="crafting" label="Crafting">
			<template slot-scope="scope">
				<div class="whitesSace">{{scope.row.crafting}}</div>
			</template>
		</el-table-column> 
	</el-table>
...
export default {
	...
	methods: {
		...
		getDataList(params) {//获取列表
			...
			this.$http({
              url: this.$http.adornUrl('/user/bagpackHistoryList'),
              method: 'post',
              data: this.$http.adornData(params)
            }).then(({data}) => {
              if (data && data.code === 20000) {
