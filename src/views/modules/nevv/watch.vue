<template>
	...
	<el-table class="userTable" border :data="dataList" highlight-current-row v-loading="listLoading"> 
		...
		<el-table-column prop="watchTitle" label="Title"></el-table-column> 
		<el-table-column prop="eventWatchImg" label="Icon">
			<template slot-scope="scope">
				<img class="listImg" :src="scope.row.eventWatchImg" @click="showPreviewImage(scope.row.eventWatchImg)" />
			</template>
		</el-table-column> 
		...
		<el-table-column prop="isBanner" label="Banner">
			<template slot-scope="scope">
				<el-switch v-model="scope.row.isBanner" :active-value="1" :inactive-value="0" @change="switchHandle(scope.row.id,scope.row.isBanner)"></el-switch>
			</template>
		</el-table-column>
...
<script>
import AddOrUpdate from './watch-update'
export default {
	data() {
		return {
			filters: {
	...
	methods: {
		...
		deleteHandle(id){
			this.$confirm('This operation will permanently delete the file, do you want to continue?', 'Prompt', {
				...
				this.$http({
					url: this.$http.adornUrl('/eventWatch/pc/delWatchInfo'),
					method: 'post',
					data: this.$http.adornData(params)
				}).then(({data}) => {
					if (data && data.code === 20000) {
		...
		getDataList(params) {
			...
			this.$http({
              url: this.$http.adornUrl('/eventWatch/pc/findWatchList'),
              method: 'post',
              data: this.$http.adornData(params)
            }).then(({data}) => {
              if (data && data.code === 20000) {
